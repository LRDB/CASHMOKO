...
class Sprint2TestBasis(TestCase):
    ...
    def setUp(self):
        ...
        self.new_user_login_correct={
            'username':'nina',
            'password':'sapitula'
        }
        self.new_user_login_wrong={
            'username':'nina',
            'password':'spatula'
        }
...
class RegistrationTests(Sprint2TestBasis):
    ...
    def test_signup_username_already_taken(self):
        response=self.client.post(self.signup_url,self.new_user_same_username,format='text/html')
        self.assertEqual(response.status_code, 302)
    
    def test_signup_email_already_taken(self):
        response=self.client.post(self.signup_url,self.new_user_same_email,format='text/html')
        self.assertEqual(response.status_code, 302)
    ...
    def test_login_with_new_account(self):
        ...
        response=self.client.post(self.login_url,self.new_user_login_correct,format='text/html')
        self.assertEqual(response.status_code, 200)
