...
class Sprint3TestBasis(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="hashimoto",email="real@email.com",password="kanna")
        categories = {
            "deposit": {
                "Allowance": "Allowance",
...
class TransactionFormTest(Sprint3TestBasis):
    ...
    def test_CreateTransactionEntry_Dr(self):
        ...
        form = CreateTransactionEntry(user=self.user, cat="dep_category", data=form_data)
        self.assertTrue(form.is_valid())
    ...
    def test_CreateTransactionEntry_Cr(self):
        ...
        form = CreateTransactionEntry(user=self.user, cat="cred_category", data=form_data)
        self.assertTrue(form.is_valid())
        
    def test_CreateTransactionEntry_CrERROR(self):
        form_data = {"amount": 69, "endBank": "Maya", "category": "Hardcore Illegal Drugs"}
        form = CreateTransactionEntry(user=self.user, cat="cred_category", data=form_data)
        self.assertFalse(form.is_valid())
